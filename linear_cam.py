...
# 하드코딩 설정
# =========================p
WEIGHTS_PATH   = r"C:\Users\ust21\label\final_test\Busan_DL.h5"
IMAGE_FOLDER   = r"C:\Users\ust21\label\final_test\Busan\auto_1\2"  # 이미지 폴더
...
# linear-CAM
# =========================
def make_linear_cam(model, inputs, target_layer_name=TARGET_LAYER_NAME, class_index=None):
    """
    Linear-CAM:
    마지막 Conv 특성맵 A_k(x,y)들을 최종 Dense의 클래스별 가중치 w_k^(c)로
    선형 결합하여 CAM을 만듭니다. (gradient 불필요)
    """
    # (a) 마지막 Conv 출력
    target_layer = model.get_layer(target_layer_name)
    conv_model = tf.keras.models.Model(model.inputs, target_layer.output)
    conv_out = conv_model(inputs, training=False).numpy()   # (1, Hc, Wc, C)

    # (b) 예측 (class index)
    preds = model(inputs, training=False).numpy()           # (1, num_classes)
    if class_index is None:
        class_index = int(np.argmax(preds[0]))

    # (c) Dense 가중치 (shape: (C, num_classes))
    dense = model.get_layer("dense")
    W, b = dense.get_weights()                              # W: (C, K), b: (K,)

    # (d) 선형 결합  cam(x,y) = ReLU( Σ_k A_k(x,y) * w_k^(c) )
    Ak  = conv_out[0]                                       # (Hc, Wc, C)
    w_c = W[:, class_index]                                 # (C,)
    cam = np.tensordot(Ak, w_c, axes=([2], [0]))            # (Hc, Wc)
...
def main():
    ...
    while 0 <= idx < len(image_files):
        ...
        heatmap, cls_idx, preds = make_linear_cam(model, x, TARGET_LAYER_NAME, class_index=None)
