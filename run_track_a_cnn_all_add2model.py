...
TRAIN_PY = "/home/wj/seafog/src/train_add2model.py"
...
def build_jobs(backbones: Sequence[str], ports: Sequence[str]) -> List[Tuple[str, str]]:
...
def aggregate_backbone(backbone: str, ports: Sequence[str]) -> Dict:
    rows = []
    for port in ports:
        out_dir = RESULT_ROOT / backbone / port
...
def main() -> None:
    ...
    jobs = build_jobs(backbones, ports)
    total_jobs = len(jobs)
    completed_before = count_completed_jobs(jobs)

    print("=" * 100, flush=True)
    print("Track A | Added transformer backbones x selected ports sequential run", flush=True)
    ...
    for i, (backbone, port) in enumerate(jobs, start=1):
        ...
        run_one(backbone, port, job_idx=i, total_jobs=total_jobs, skip_completed=args.skip_completed)
        ...
        if port == ports[-1]:
            row = aggregate_backbone(backbone, ports)
    ...
    print("=" * 100, flush=True)
    print(f"[{ts()}] ALL ADDED BACKBONE RUNS FINISHED", flush=True)
