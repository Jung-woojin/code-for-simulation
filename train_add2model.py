...
def build_model(
    ...
    if img_size is not None and (backbone.startswith("vit_") or backbone.startswith("swin_")):
        model_kwargs["img_size"] = img_size

    return timm.create_model(backbone, **model_kwargs)
...
def infer_group_name(backbone: str, param_name: str) -> str:
    ...
    if backbone.startswith("vit_"):
        if param_name.startswith(("cls_token", "pos_embed", "patch_embed", "norm_pre")):
            return "first"
        if param_name.startswith(("blocks.0", "blocks.1", "blocks.2", "blocks.3")):
            return "first"
        if param_name.startswith(("blocks.4", "blocks.5", "blocks.6", "blocks.7")):
            return "mid"
        if param_name.startswith(("blocks.8", "blocks.9", "blocks.10", "blocks.11", "norm")):
            return "last"
        return "mid"

    # Swin
    if backbone.startswith("swin_"):
        if param_name.startswith(("patch_embed", "layers.0")):
            return "first"
        if param_name.startswith(("layers.1", "layers.2")):
            return "mid"
        if param_name.startswith(("layers.3", "norm")):
            return "last"
        return "mid"
